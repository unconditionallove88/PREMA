...
export default function BeforePhase() {
  ...
  const sections = [
    { 
      ...
      action: () => setLocation('/laboratory-test') 
    },
    { 
      id: 'hydration',
  ...
  return (
    <main className="min-h-screen bg-background text-foreground font-headline pb-32 relative overflow-x-hidden pt-safe">
      <header className="px-6 py-8 border-b border-border bg-card/90 backdrop-blur-xl flex items-center gap-4 sticky top-0 z-50">
        <button onClick={() => setLocation("/dashboard")} className="p-3 bg-card border border-border rounded-full"><ArrowLeft className="w-5 h-5 text-muted-foreground" /></button>
        <div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 h-[100px] bg-card/90 backdrop-blur-xl border-t border-border flex items-center justify-center px-6 z-50 pb-safe">
        <button onClick={() => setLocation("/dashboard")} className="w-full max-sm py-6 bg-primary text-primary-foreground rounded-full font-semibold uppercase text-lg tracking-[0.1em] shadow-soft active:scale-95 transition-all">{t.button}</button>
      </footer>
    </main>
  );
}
