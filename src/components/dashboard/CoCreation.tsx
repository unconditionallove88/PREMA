...
const i18n = {
  ...
  de: {
    ...
    types: [
      ...
      { key: "survey", label: "App Umfrage", prompt: "Hilf uns diesen Raum zu testen", placeholder: "Nimm an unserer Umfrage teil um die Resonanz zu kalibrieren", icon: CircleDot, color: "text-foreground", bg: "bg-card/10" },
    ],
    ...
    shareMore: "Mehr teilen", receivedWithLove: "Mit Liebe empfangen"
  }
};
...
export function CoCreation({ onComplete }: { onComplete?: () => void }) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isSurvey) { window.open(SURVEY_LINK, '_blank'); return; }
    if (!message.trim() || !firestore) return;
    setLoading(true);
    try {
      addDocumentNonBlocking(collection(firestore, "feedback"), {
        userId: user?.uid || "anonymous", language: lang, type: active.key, message: message.trim(), status: "heard", createdAt: serverTimestamp(),
      });
      setSent(true); setMessage("");
    } catch (error) { console.error("Feedback error:", error); } finally { setLoading(false); }
  };
      ...
      <header className="px-8 pt-10 pb-6 border-b border-border/5 shrink-0 bg-card/95 backdrop-blur-xl z-50">
        <div className="flex items-center justify-between max-w-2xl mx-auto w-full">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-foreground font-black text-xl uppercase tracking-tighter leading-none">{t.title}</h3>
          ...
          {sent ? (
            <div className="w-full min-h-[400px] flex flex-col items-center justify-center p-10 text-center animate-in fade-in zoom-in-95 duration-500 font-headline">
              ...
              <div className="flex items-center justify-center gap-3 mb-4">
                <h3 className="text-foreground font-black text-3xl uppercase tracking-tighter">{t.successTitle}</h3>
              ...
              <p className="text-foreground/80 text-base font-bold leading-relaxed max-w-xs mx-auto mb-10">{t.successMsg}</p>
              <button onClick={() => setSent(false)} className="text-[10px] font-black uppercase text-primary tracking-[0.4em] hover:text-white transition-colors underline underline-offset-8">{t.shareMore}</button>
            </div>
          ) : (
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {t.types.map((type, i) => {
                  ...
                  return (
                    <button key={type.key} onClick={() => { setActiveType(i); setMessage(""); }} className={cn("p-4 rounded-2xl border-2 flex flex-col items-center justify-center gap-3 transition-all duration-300 h-24", isSelected ? `${type.bg} ${type.color.replace('text-', 'border-')} shadow-lg` : "bg-card/5 border-border/5 hover:border-border/20")}><TypeIcon size={20} className={isSelected ? type.color : "text-foreground/20"} /><span className={cn("text-[9px] font-black uppercase tracking-widest text-center leading-none", isSelected ? type.color : "text-muted-foreground")}>{type.label}</span></button>
                  );
                })}
              </div>

              <form onSubmit={handleSend} className="space-y-8">
                {!isSurvey ? (<textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder={active.placeholder} className="w-full h-48 px-8 py-6 rounded-[2.5rem] border-2 border-border/10 bg-card/5 text-white text-lg font-bold outline-none resize-none focus:border-primary transition-all shadow-inner" required />) : (<div className="w-full h-48 px-8 py-10 rounded-[2.5rem] border-2 border-dashed border-border/10 bg-card/5 flex flex-col items-center justify-center text-center gap-4"><CircleDot className="text-foreground/20" size={40} /><p className="text-muted-foreground text-sm font-bold uppercase tracking-widest leading-relaxed">{active.placeholder}</p></div>)}
